...
async function saveIotData(sensorType, value) {
  const session = JSON.parse(sessionStorage.getItem('catUser'));
  if (!session) return;
  try {
    await fetch(`${API_BASE}/iot`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: session.id, sensorType, value })
    });
  } catch(e) { console.error("IoT data save error", e); }
}

window.loadIotData = async function() {
  const session = JSON.parse(sessionStorage.getItem('catUser'));
  if (!session) return [];
  try {
    const res = await fetch(`${API_BASE}/iot/user/${session.id}`);
...
function stopMQTTSimulator() {
  ...
  saveIotData('MQTT Telemetry', `Logged ${mqttDataSets.labels.length} points. Final Temp: ${mqttDataSets.temp.slice(-1)[0]}°C`);
}
